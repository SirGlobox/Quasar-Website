...
const GooeyNav = ({
  ...
  const makeParticles = (element) => {
    if (isMobile) return; // Skip particle effects on mobile
  ...
  const updateEffectPosition = (element) => {
    if (!containerRef.current || !filterRef.current || !textRef.current || isMobile)
      return;
  ...
  useEffect(() => {
    if (!navRef.current || !containerRef.current || isMobile) return;
  ...
  return (
    <div className={`gooey-nav-container ${className} ${isMobile ? 'mobile' : ''}`} ref={containerRef}>
      <nav>
        <ul ref={navRef}>
